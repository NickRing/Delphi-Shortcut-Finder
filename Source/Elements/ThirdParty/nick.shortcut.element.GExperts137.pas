unit nick.shortcut.element.GExperts137;
...
begin
  ...
  if (not LRegistryRepository.OpenKeyReadOnly(ABaseRegistryKey + 'Misc')) then
    Exit;
...
begin
  ...
  if (LRegistryRepository.OpenKeyReadOnly(LBaseRegistryKey + 'Misc')) then
  try
    if (LRegistryRepository.ValueExists('PlaceGxMainMenuInToolsMenu')) and
...
initialization
  nick.shortcut.repository.ShortCut.GetShortCutRepository().Add(TGExperts137Element);
